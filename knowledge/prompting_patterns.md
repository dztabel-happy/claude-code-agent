...
看到触发器之后，不要模糊地“整体升级”，而是优先做最匹配的升级：

| 信号 | 优先升级项 | 说明 |
|------|-----------|------|
| 任务理解难、推理链长 | `model` / `effort` | 先增强思考能力，不一定要换会话模式 |
| 需要多轮推进、等待反馈、持续监督 | tmux 托管会话 | 这是会话形态问题，不只是模型问题 |
| 需要浏览器、联网、站点交互 | Chrome / WebSearch / WebFetch | 这是工具问题，不应只靠更强模型硬扛 |
| 需要隔离改动、并行试验、避免污染主树 | worktree | 先隔离环境，再决定是否并行 |
| 任务天然可拆分、职责边界清晰 | Agent Teams | 只有拆分收益显著时才开 |
| 输出格式要求严格 | `--json-schema` / 结构化输出规则 | 先约束输出，再谈更强模型 |
| 默认权限太保守导致频繁卡住 | 权限模式 / `--allowed-tools` | 先精细授权，不要直接危险跳权 |

## 最小足够升级原则

OpenClaw 每次升级都应尽量只升级一层主矛盾：

- 推理不够，就先升模型或 effort
- 工具不够，就先补工具
- 会话不够，就先换成 tmux
- 环境不够，就先加 worktree
- 并行才有明显收益时，才升到 Agent Teams

不要因为任务复杂，就默认把“高模型 + tmux + Chrome + worktree + Agent Teams”一起开满。

## 升级后的复核问题

每次决定升级前，OpenClaw 至少问自己这几个问题：

- 我升级的是当前真正的瓶颈吗？
- 有没有更便宜、更简单的升级方式先试？
- 这次升级会不会增加不必要的协调成本？
- 升级后，我的验证和汇报要求是否也同步提高了？
