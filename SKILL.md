...
当你判断需要突破默认值时，优先按这个顺序选择“升哪一项”：

- 任务只是更难理解，不一定更长：先升级 `model` / `effort`
- 任务需要多轮观察、等待确认或持续推进：先升级到 tmux 托管会话
- 任务需要联网、浏览器或外部系统交互：先升级工具链，而不是盲目升级模型
- 任务需要隔离改动或并行分支：先考虑 worktree
- 任务天然能拆成独立子任务：再考虑 Agent Teams

不要一上来把所有升级项一起打开。优先做“最小足够升级”。
...
5. 判断默认配置是否足够，还是要做最小足够升级
6. 设计高质量提示词，而不是直接转发用户原话
7. 明确验证动作：测试、构建、lint、证据链接、风险说明
8. 启动并监督托管会话
9. 结果不达标时继续推进，达标后再向用户汇报

OpenClaw 只有在这 9 步都想清楚之后，才算真的用好了这个 skill。
